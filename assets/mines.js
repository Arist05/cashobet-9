...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-a1c4b0fa";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-a1c4b0fa">'+'<span class="tile-face-a1c4b0fa tile-front-a1c4b0fa"></span>'+'<span class="tile-face-a1c4b0fa tile-back-a1c4b0fa"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-a1c4b0fa")){return;}
var back=tile.querySelector(".tile-back-a1c4b0fa");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-a1c4b0fa");if(isMine){tile.classList.add("boom-a1c4b0fa");}
if(dim){tile.classList.add("dimmed-a1c4b0fa");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-a1c4b0fa")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-a1c4b0fa");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-a1c4b0fa")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-a1c4b0fa");void panelEl.offsetWidth;panelEl.classList.add("jolt-a1c4b0fa");}
endRound();}
